...
# probably will rework how handMode works, probably also give it a more accurate name
  ...
  if handMode == "3":
    ...
    # yeah this is ugly. i want to make this better but idk how
    startingColor = startingCard.color
    startingAction = startingCard.action
            ...
            if "WILD" in card.action:
              ...
              for CARD in opponentHand:
                if CARD.color == "red":
                  ...
                  yellows.append(CARD)
              colors = ["red", "blue", "green", "yellow"]
              amountsOfColors = [len(reds), len(blues), len(greens), len(yellows)]
              amountsOfColorsSorted = [len(reds), len(blues), len(greens), len(yellows)]
